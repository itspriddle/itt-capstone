...
<form name="searchform" action="" method="get" style="float: left; width: 16em; margin-right: 3em;"> 
  <fieldset> 
  <legend><?php _e('Search Entries&hellip;') ?></legend> 
...
if ( isset( $_GET['c'] ) ) {
	...
	switch( $thePage ) {
		...
		case 'news':
		default:
			$thePageCat = 'Site News';
			break;
	}

	if ( trim( $categories[0]->cat_name ) != $thePageCat ) continue;
} else {
//	if ( trim( $categories[0]->cat_name ) != 'Site News' ) continue;
}
...
foreach($posts_columns as $column_name=>$column_display_name) {
	...
	case 'categories':
		?>
		<td><?php /* the_category(',');*/ echo $categories[0]->cat_name; ?></td>
		<?php
		break;
	...
	case 'control_edit':
		?>
		<td><?php if ( current_user_can('edit_post',$post->ID) ) { echo "<a href='". ( ( $categories[0]->cat_name == 'Reviews' ) ? 'reviews' : 'post' ) .".php?action=edit&amp;post=$id' class='edit'>" . __('Edit') . "</a>"; } ?></td>
		<?php
		break;
...
<div class="alignleft"><?php /*if ( $postsTotal > 15 )*/ next_posts_link(__('&laquo; Previous Entries')) ?></div>
<div class="alignright"><?php /*if ( $postsTotal > 15 )*/ previous_posts_link(__('Next Entries &raquo;')) ?></div>
</div>
