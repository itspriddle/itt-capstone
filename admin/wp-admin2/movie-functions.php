...
require_once "/home/priddle/www/inc/imdb/imdbsearch.class.php";
 ...
 */
	...
	foreach( get_all_titles( "/home/priddle/www/movies/", FALSE ) as $title ) {
		$sql = "SELECT * FROM movies WHERE filename = '". addslashes( $title ). "'";
 ...
 */
	$sql = "SELECT * FROM movies WHERE imdb_id = 0";
	$res = mysql_query( $sql );
 ...
 */
	$sql = "SELECT * FROM movies WHERE have_details != 'Y' AND imdb_id > 0";
	$res = mysql_query( $sql );
 ...
 * get the genre ID
 */
	$sql = "SELECT genre_id FROM genres WHERE genre_name = '". trim( addslashes( $genre_title ) ) ."'";
	//echo $sql;
	$res = mysql_query ( $sql );
	while ( $r = mysql_fetch_array( $res ) ) {
		$genre_id = $r['genre_id'];
 ...
 */
	$sql = "SELECT filename, movie_id FROM movies WHERE imdb_id = 0";
	$res = mysql_query( $sql );
...
function add_new( ) {
	...
	if ( count( $new ) > 0 ) {
		foreach ( $new as $n ) {
			$sql =  "INSERT INTO movies (imdb_id, filename, have_details) VALUES ('0000000', ". format_sql( $n ) .
					",'n')";
			$res = mysql_query( $sql );
			
			if ( $res ) 
				$html['msg'] .= "<div id=\"message\" class=\"updated\"><p><br>$n</b> added to database! </p></div>\n";
...
// 11/25 - this shit needs work...
	 ...
	 * step 1
	 * 
	 * retrieve all movies with an imdb_id
	 */

	$sql_all = "SELECT * FROM movies WHERE have_details != 'Y' AND imdb_id > 0";
	...
	if ( mysql_num_rows( $res_all ) > 0 ) {
		...
		while ( $r = mysql_fetch_array( $res_all ) ) {
			...
			foreach ( $all_genres as $g ) {
				...
				// Update movie_genres
				$mg_sql = "INSERT INTO movie_genres (movie_id, genre_id) VALUES ".
			...
			$md_sql =   "INSERT INTO movie_details ( ".
						" movie_id, date_added, name, year, runtime, genre_id ".
						" ) VALUES ( ".
						format_sql( $movie_id ). ','.
						' now(), '.
						format_sql( $title ). ','.
						...
						format_sql( $runtime ). ','.
						format_sql( $genreID ). 
						//format_sql( $short_plot )
						")";
			
			$ratingSQL = "INSERT INTO movie_ratings (id) VALUES ($movie_id)";
...
function get_movie_genres( $mid ) {
    $sql = "SELECT genre_id FROM movie_genres WHERE movie_id = $mid";
    $res = mysql_query( $sql );
    if ( $res ) {
        while ( $r = mysql_fetch_array( $res ) ) {
            $genreIDs[] = $r['genre_id'];
...
function editmovielist( $start = NULL, $perpage = NULL ) {
    $sqlMovies = "SELECT * FROM movies WHERE have_details = 'Y' ORDER BY filename";
    $resMovies = mysql_query( $sqlMovies );
	$c = 0;
    while ( $r = mysql_fetch_array( $resMovies ) ) {
        $movie = substr( $r['filename'], 0, ( strlen( $r['filename'] ) - 4 ) );
