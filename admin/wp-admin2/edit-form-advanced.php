...
<form name="post" action="<?php echo ( ( strstr( $_SERVER['PHP_SELF'], 'reviews.php' ) ) ? 'reviews.php' : 'post.php' ) ?>" method="post" id="post">
...
<!--
        <div class="wrap">
          <h2>Instructions</h2>
          <p>Add content to your site's pages below.  Choose the page you would like to add
          content to from the right.</p>
          <p>You can upload images using the Upload form below.  To insert images, simply click on the thumbnail,
          and then choose 'Send to editor'.  Now, you can drag the image wherever you would like.
          </p>
	  <p>You can click 'Save' to save an entry you need more time to work on.  If you're finished up, click
	  'Publish' to add the entry to your site now.</p>
        </div>
-->

<div class="wrap">
<h2 id="write-post">
<?php if ( strstr( $_SERVER['PHP_SELF'], 'reviews.php' ) ) 
	_e('Add Reviews');
else _e('Add News');
?>
...
-->
<fieldset id="categorydiv" class="dbx-box" style="height: 1px; visibility: hidden;">
<h3 class="dbx-handle"><?php _e('Pages') ?></h3>
<div class="dbx-content">
<p id="jaxcat"></p>
<?php if ( strstr( $_SERVER['PHP_SELF'], 'reviews.php' ) ) : ?>
<input type="hidden" name="post_category[]" value="2" />
<?php else : ?>
...
<fieldset class="dbx-box">
<h3 class="dbx-handle"><?php _e('Entry Timestamp'); ?>:</h3>
...
<h3 class="dbx-handle"><?php _e('Entry author'); ?>:</h3>
<div class="dbx-content">
...
<input name="deletepost" class="button" type="submit" id="deletepost" tabindex="10" value="<?php _e('Delete this entry') ?>" <?php echo "onclick=\"if ( confirm('" . sprintf(__("You are about to delete this entry \'%s\'\\n  \'Cancel\' to stop, \'OK\' to delete."), addslashes($post->post_title) ) . "') ) { document.forms.post._wpnonce.value = '$delete_nonce'; return true;}\""; ?> />
<?php endif; ?>
