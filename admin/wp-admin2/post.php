...
case 'post':
	...
	switch($_POST['mode']) {
		...
		if ( strstr( $_SERVER['HTTP_REFERER'], 'reviews.php' ) )
			$location = 'reviews.php?posted=true';
		else 
...
default:
	...
	require_once ('./admin-header.php');
	$cust_instructions = <<<POST_INSTRUCTIONS
	<div class="wrap">
	  <h2>Instructions</h2>
...
<?php //if ( $cust_instructions ) echo $cust_instructions; ?>
		...
		if ( $drafts = get_users_drafts( $user_ID ) ) {
			...
			for ( $i = 0; $i < $num_drafts; $i++ ) {
				...
				if ( empty($draft->post_title) )
					$draft->post_title = sprintf(__('Entry # %s'), $draft->ID);
