...
/*
   ...
   function pagination($query, $perPage, $current, $maxPage = 4, $delimiter = '>') 
...
class pagination
{
		...
		var $urlBase = '/movies/browse/';

        function pagination($query, $perPage, $current, $maxPage = 4, $delimiter = '|', $filter = NULL) {
                $this->query     = $query;
        ...
        function buildNumbers($string = '') {
                for ($x = $this->BoundaryMin; $x <= $this->BoundaryMax; $x++) {
                        ...
                        if ($this->current == $x) {
                                $string .= '<span style="font-weight: bold">'.$x.'</span> '.$this->delimiter.' ';
                        }
                        else {
                                $string .= '<a href="'. $this->urlBase .$x.'">'.$x.'</a> '.$this->delimiter.' ';
                        }       
        ...
        function buildOutput() {
                $output  = ($this->current == 1 ? 'Prev '.$this->delimiter.' ' : '<a href="'. $this->urlBase . ($this->current - 1).'">Prev</a> '.$this->delimiter.' ');
                $output .= ($this->current == 1 ? 'First '.$this->delimiter.' ' : '<a href="'. $this->urlBase .'1">First</a> '.$this->delimiter.' ');
                $output .= $this->numbers;
                $output .= ($this->current == ($this->totalNumPages) ? 'Next '.$this->delimiter.' ' : '<a href="'. $this->urlBase .($this->current + 1).'" >Next</a> '.$this->delimiter.' ');               
                $output .= ($this->current == ($this->totalNumPages) ? 'Last ' : '<a href="'. $this->urlBase .($this->totalNumPages).'">Last</a>');
                return $output;
        }
